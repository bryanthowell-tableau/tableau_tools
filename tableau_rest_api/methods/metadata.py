...
class MetadataMethods35():
    ...
    def query_databases(self) -> ET.Element:
        ...
        response = self.rest.query_resource("databases")
    ...
    def query_database(self, database_name_or_luid: str) -> ET.Element:
        ...
        response = self.rest.query_single_element_from_endpoint("database", name_or_luid=database_name_or_luid)
        self.rest.end_log_block()
        return response
    ...
    def update_database(self, database_name_or_luid: str, certification_status: Optional[bool] = None,
        ...
        if contact_username_or_luid is not None:
            user_luid = self.rest.rest_api_base.query_user_luid(username=contact_username_or_luid)
            c = ET.Element('contact')
            c.set('id', user_luid)
            d.append(c)
        tsr.append(d)
        database_luid = self.rest.query_database_luid(database_name=database_name_or_luid)
        url = self.rest.build_api_url("databases/{}".format(database_luid))
        response = self.rest.send_update_request(url=url, request=tsr)
        self.rest.end_log_block()
        return response
    ...
    def remove_database(self, database_name_or_luid: str):
        ...
        database_luid = self.rest.query_database_luid(database_name=database_name_or_luid)
        url = self.rest.build_api_url("databases/{}".format(database_luid))
        self.rest.rest_api_base.send_delete_request(url)
        self.rest.end_log_block()

    def query_tables(self) -> ET.Element:
        self.rest.start_log_block()
        response = self.rest.query_resource("tables")
        self.rest.end_log_block()
        return response
    ...
    def query_table(self, table_name_or_luid: str) -> ET.Element:
        self.rest.start_log_block()
        # Implement the search mechanism eventually using XPath similar to old workbooks / datasources lookup
        response = self.rest.query_single_element_from_endpoint("table", name_or_luid=table_name_or_luid)
        self.rest.end_log_block()
        return response
    ...
    def update_table(self, table_name_or_luid: str, certification_status: Optional[bool] = None,
        ...
        if contact_username_or_luid is not None:
            ...
            t.append(c)
        tsr.append(t)
        table_luid = self.rest.query_database_luid(database_name=table_name_or_luid)
        url = self.rest.build_api_url("tables/{}".format(table_luid))
        response = self.rest.send_update_request(url=url, request=tsr)
        self.rest.end_log_block()
        return response
    ...
    def remove_table(self, table_name_or_luid: str):
        self.rest.start_log_block()
        table_luid = self.rest.query_table_luid(table_name=table_name_or_luid)
        url = self.rest.build_api_url("tables/{}".format(table_luid))
        self.rest.rest_api_base.send_delete_request(url)
        self.rest.end_log_block()
    ...
    def query_columns_in_a_table(self, table_name_or_luid: str) -> ET.Element:
        self.rest.start_log_block()
        table_luid = self.rest.query_table_luid(table_name_or_luid)
        response = self.rest.query_resource("tables/{}/columns".format(table_luid))
        self.rest.end_log_block()
        return response
    ...
    def update_column(self, table_name_or_luid: str, column_name_or_luid: str,
        ...
        tsr.append(c)
        table_luid = self.rest.query_table_luid(table_name=table_name_or_luid)
        column_luid = self.rest.query_column_luid(database_name=column_name_or_luid)
        url = self.rest.build_api_url("tables/{}/columns/{}".format(table_luid, column_luid))
        response = self.rest.send_update_request(url=url, request=tsr)
        self.rest.end_log_block()
        return response
    ...
    def add_data_quality_warning(self, content_type: str, content_luid: str) -> ET.Element:
        ...
        url = self.rest.build_api_url("dataQualityWarnings/{}/{}".format(content_type, content_luid))
        response = self.rest.send_post_request(url)
        self.rest.end_log_block()
        return response

    def query_data_quality_warning_by_id(self, data_quality_warning_luid: str) -> ET.Element:
        self.rest.start_log_block()
        url = self.rest.build_api_url("dataQualityWarnings/{}".format(data_quality_warning_luid))
        response = self.rest.query_resource(url)
        self.rest.end_log_block()
        return response
    ...
    def delete_data_quality_warning_by_content(self, content_type: str, content_luid: str):
        self.rest.delete_data_quality_warning_by_asset(content_type=content_type, content_luid=content_luid)
    ...
    def graphql(self, graphql_query: str) -> Dict:
        ...
        url = self.rest.rest_api_base.build_api_url("metadata")
        try:
            response = self.rest.rest_api_base.send_add_request_json(url, graphql_json)
            self.rest.end_log_block()
            return response
