...
class UrlFilter:
    ...
    @staticmethod
    def create_name_filter(name: str) -> 'UrlFilter':
        return UrlFilter('name', 'eq', [name, ])

    # Users
    @staticmethod
    def create_last_login_filter(operator: str, last_login_time: str) -> 'UrlFilter':
        """
    ...
    # Users
    @staticmethod
    def create_site_role_filter(site_role: str) -> 'UrlFilter':
        return UrlFilter('siteRole', 'eq', [site_role, ])

    # Workbooks
    @staticmethod
    def create_owner_name_filter(owner_name: str) -> 'UrlFilter':

        return UrlFilter('ownerName', 'eq', [owner_name, ])

    # Workbooks, Datasources, Views, Jobs
    @staticmethod
    def create_created_at_filter(operator: str, created_at_time: str) -> 'UrlFilter':
        """
        :param operator: Should be one of 'eq', 'gt', 'gte', 'lt', 'lte'
    ...
    @staticmethod
    def create_updated_at_filter(operator: str, updated_at_time: str) -> 'UrlFilter':
        """
        :param operator: Should be one of 'eq', 'gt', 'gte', 'lt', 'lte'
    ...
    # Workbooks, Datasources, Views
    @staticmethod
    def create_tags_filter(tags: List[str]) -> 'UrlFilter':
        return UrlFilter('tags', 'in', tags)

    # Workbooks, Datasources, Views
    @staticmethod
    def create_tag_filter(tag: str) -> 'UrlFilter':
        return UrlFilter('tags', 'eq', [tag, ])

    # Datasources
    @staticmethod
    def create_datasource_type_filter(ds_type: str) -> 'UrlFilter':
...
class UrlFilter27(UrlFilter):
    ...
    # Users, Datasources, Views, Workbooks
    @staticmethod
    def create_names_filter(names: List[str]) -> 'UrlFilter':
        return UrlFilter('name', 'in', names)

    # Users
    @staticmethod
    def create_site_roles_filter(site_roles: List[str]) -> 'UrlFilter':
        return UrlFilter('siteRole', 'in', site_roles)

    # Workbooks, Projects
    @staticmethod
    def create_owner_names_filter(owner_names: List[str]) -> 'UrlFilter':
    ...
    # Groups
    @staticmethod
    def create_domain_names_filter(domain_names: List[str]) -> 'UrlFilter':
        return UrlFilter('domainName', 'in', domain_names)

    # Groups
    @staticmethod
    def create_domain_nicknames_filter(domain_nicknames: str) -> 'UrlFilter':

        return UrlFilter('domainNickname', 'in', domain_nicknames)

    # Groups
    @staticmethod
    def create_domain_name_filter(domain_name: str) -> 'UrlFilter':
        return UrlFilter('domainName', 'eq', [domain_name, ])

    # Groups
    @staticmethod
    def create_domain_nickname_filter(domain_nickname: str) -> 'UrlFilter':

        return UrlFilter('domainNickname', 'eq', [domain_nickname, ])

    # Groups
    @staticmethod
    def create_minimum_site_roles_filter(minimum_site_roles: List[str]) -> 'UrlFilter':
        return UrlFilter('minimumSiteRole', 'in', minimum_site_roles)

    # Groups
    @staticmethod
    def create_minimum_site_role_filter(minimum_site_role: str) -> 'UrlFilter':
        return UrlFilter('minimumSiteRole', 'eq', [minimum_site_role, ])

    # Groups
    @staticmethod
    def create_is_local_filter(is_local: bool) -> 'UrlFilter':
    ...
    # Groups
    @staticmethod
    def create_user_count_filter(operator, user_count: int) -> 'UrlFilter':
        comparison_operators = ['eq', 'gt', 'gte', 'lt', 'lte']
        if operator not in comparison_operators:
            raise InvalidOptionException("operator must be one of 'eq', 'gt', 'gte', 'lt', 'lte' ")
        # Convert to the correct time format
    ...
    # Projects
    @staticmethod
    def create_owner_domains_filter(owner_domains: List[str]) -> 'UrlFilter':
        return UrlFilter('ownerDomain', 'in', owner_domains)

    # Projects
    @staticmethod
    def create_owner_domain_filter(owner_domain: str) -> 'UrlFilter':
        return UrlFilter('ownerDomain', 'in', [owner_domain, ])

    # Projects
    @staticmethod
    def create_owner_emails_filter(owner_emails: List[str]) -> 'UrlFilter':
        return UrlFilter('ownerEmail', 'in', owner_emails)

    # Projects
    @staticmethod
    def create_owner_email_filter(owner_email: str) -> 'UrlFilter':
        return UrlFilter('ownerEmail', 'in', [owner_email, ])

    # Views
    @staticmethod
    def create_hits_total_filter(operator, hits_total: int) -> 'UrlFilter':
        comparison_operators = ['eq', 'gt', 'gte', 'lt', 'lte']
        if operator not in comparison_operators:
            raise InvalidOptionException("operator must be one of 'eq', 'gt', 'gte', 'lt', 'lte' ")
        # Convert to the correct time format
...
class UrlFilter31(UrlFilter30):
    ...
    # Jobs
    @staticmethod
    def create_started_at_filter(operator: str, started_at_time: str) -> UrlFilter:
        """
        :param operator: Should be one of 'eq', 'gt', 'gte', 'lt', 'lte'
    ...
    # Jobs
    @staticmethod
    def create_ended_at_filter(operator: str, ended_at_time: str) -> UrlFilter:
        """
        :param operator: Should be one of 'eq', 'gt', 'gte', 'lt', 'lte'
    ...
    # Jobs
    @staticmethod
    def create_job_types_filter(job_types: List[str]) -> UrlFilter:
        return UrlFilter('jobType', 'in', job_types)

    # Jobs
    @staticmethod
    def create_job_type_filter(job_type: str) -> UrlFilter:
        return UrlFilter('tags', 'eq', [job_type, ])

    # Jobs
    @staticmethod
    def create_notes_filter(notes: str) -> UrlFilter:
        return UrlFilter('notes', 'has', [notes, ])

    @staticmethod
    def create_title_equals_filter(title: str) -> UrlFilter:
        return UrlFilter('title', 'eq', [title, ])

    @staticmethod
    def create_title_has_filter(title: str) -> UrlFilter:
        return UrlFilter('title', 'has', [title, ])

    @staticmethod
    def create_subtitle_equals_filter(subtitle: str) -> UrlFilter:
        return UrlFilter('subtitle', 'eq', [subtitle, ])

    @staticmethod
    def create_subtitle_has_filter(subtitle: str) -> UrlFilter:
...
class UrlFilter33(UrlFilter31):
    ...
    @staticmethod
    def create_project_name_equals_filter(project_name: str) -> UrlFilter:
